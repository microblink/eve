...
int main()
{
  ...
  std::cout << "---- simd" << '\n'
            << "<- x                            = " << x << '\n'
            << "<- nu                           = " << nu  << '\n'
            << "-> diff(cyl_bessel_j)(nu, x)    = " << eve::diff(eve::cyl_bessel_j)(nu, x) << '\n'
            << "-> diff(cyl_bessel_j)(nu, -x)   = " << eve::diff(eve::cyl_bessel_j)(nu,-x) << '\n'
            << "-> diff(cyl_bessel_j)(3, x)     = " << eve::diff(eve::cyl_bessel_j)(3, x) << '\n'
            << "-> diff(cyl_bessel_j)(nu, 0.1)  = " << eve::diff(eve::cyl_bessel_j)(nu, 0.1) << '\n';

  std::cout << "---- scalar" << '\n'
            << "-> diff(cyl_bessel_j)(1.0, 3.0) = " << eve::diff(eve::cyl_bessel_j)(1.0, 3.0) << '\n';
  return 0;
}
